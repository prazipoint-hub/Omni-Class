...
export async function assignments(req, res) {
  const rows = await TeachingAssignment.findAll({ where: { teacherId: req.user.teacherId, schoolId: req.user.schoolId }, include: [Class, Subject], order: [["createdAt", "DESC"]] });
  res.json(rows);
}

export async function learners(req, res) {
  const { assignmentId } = req.params;
  const assignment = await TeachingAssignment.findOne({ where: { id: assignmentId, teacherId: req.user.teacherId, schoolId: req.user.schoolId } });
  assert(assignment, "Teaching assignment not found", 404);
  const rows = await Enrollment.findAll({ where: { schoolId: req.user.schoolId, classId: assignment.classId }, include: [{ model: Student, where: { status: "active" } }] });
...
export async function createAttendanceSession(req, res) {
  ...
  const session = await AttendanceSession.create({ schoolId: req.user.schoolId, teacherId: req.user.teacherId, classId, subjectId, date, period });
  if (records.length) {
    await AttendanceRecord.bulkCreate(records.map((record) => ({ attendanceSessionId: session.id, studentId: record.studentId, status: record.status, note: record.note || null })));
  }
...
export async function updateAttendance(req, res) {
  ...
  const session = await AttendanceSession.findOne({ where: { id: sessionId, teacherId: req.user.teacherId, schoolId: req.user.schoolId } });
  ...
  await AttendanceRecord.bulkCreate((req.body.records || []).map((record) => ({ attendanceSessionId: sessionId, studentId: record.studentId, status: record.status, note: record.note || null })));
...
export async function createAssessment(req, res) {
  const assignment = await TeachingAssignment.findOne({ where: { id: req.body.teachingAssignmentId, teacherId: req.user.teacherId, schoolId: req.user.schoolId } });
  assert(assignment, "Teaching assignment not found", 404);
...
export async function getAssessment(req, res) {
  const assessment = await Assessment.findOne({ where: { id: req.params.assessmentId, schoolId: req.user.schoolId }, include: [{ model: TeachingAssignment, where: { teacherId: req.user.teacherId }, include: [Class, Subject] }] });
...
export async function saveMarks(req, res) {
  const assessment = await Assessment.findOne({ where: { id: req.params.assessmentId, schoolId: req.user.schoolId }, include: [{ model: TeachingAssignment, where: { teacherId: req.user.teacherId } }] });
  assert(assessment, "Assessment not found", 404);
  ...
  for (const item of req.body.marks || []) {
    ...
    await Mark.upsert({ assessmentId: assessment.id, studentId: item.studentId, mark: item.mark, comment: item.comment || null, enteredBy: req.user.teacherId });
  }
...
export async function listScheme(req, res) {
  const assignment = await TeachingAssignment.findOne({ where: { id: req.params.assignmentId, teacherId: req.user.teacherId, schoolId: req.user.schoolId } });
  assert(assignment, "Teaching assignment not found", 404);
...
export async function updateSchemeItem(req, res) {
  const row = await SchemeOfWork.findOne({ where: { id: req.params.id, schoolId: req.user.schoolId }, include: [{ model: TeachingAssignment, where: { teacherId: req.user.teacherId } }] });
  assert(row, "Scheme item not found", 404);
  await row.update(req.body);
  if (req.body.status === "completed") { row.completedAt = new Date(); await row.save(); }
  res.json(row);
}
...
export async function publishHomework(req, res) {
  const homework = await Homework.findOne({ where: { id: req.params.id, schoolId: req.user.schoolId }, include: [{ model: TeachingAssignment, where: { teacherId: req.user.teacherId } }] });
  assert(homework, "Homework not found", 404);
  homework.status = "published"; await homework.save(); res.json(homework);
}

export async function markHomework(req, res) {
  const submission = await HomeworkSubmission.findOne({ where: { id: req.params.submissionId }, include: [{ model: Homework, where: { schoolId: req.user.schoolId }, include: [{ model: TeachingAssignment, where: { teacherId: req.user.teacherId } }] }] });
